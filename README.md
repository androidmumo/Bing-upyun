...
|  random   |    否    |         true/false         | 随机返回任意一天（包括当天）的图片 | 需要在`/api/php/index.php`中填写第一次访问URL的日期（见[4.1.5 设置定时任务（重要）](#4.1.5 设置定时任务（重要）)） |

注意：若参数`day` 和`random`同时存在，则参数`day` 失效，结果为随机返回任意一天（包括当天）的图片。参数`day` **或**`random`可与其余参数组合使用，除此之外的其他参数之间暂不支持组合使用。例如，不能返回灰阶的高斯模糊图片，可以返回n天前的高斯模糊图片。
